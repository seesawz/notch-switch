...
到 [**Releases**](https://github.com/seesawz/notch-switch/releases/latest) 下载 `NotchSwitch.dmg`，打开，把 App 拖进「应用程序」，启动即可。

> App 用的是自签名，没做公证。首次打开若提示「无法验证开发者」：右键 App →「打开」，或在终端执行：
>
> ```bash
> xattr -cr /Applications/NotchSwitch.app
> ```
...
**真的看得到内容**
不是应用图标，是 ScreenCaptureKit 抓的**实时窗口画面**。悬停卡片还会弹出按窗口原始比例缩放的大预览，看清再切，不用先切过去再看。
...
│   ├── 刘海几何 / 面板 / 层级守卫 / 悬停监听 / 大预览布局与焦点
│   ├── WindowKit/    窗口枚举 / MRU 排序 / 缩略图 / 窗口激活与关闭
