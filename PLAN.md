...
| 项 | 值 |
|---|---|
| 文档版本 | v1.0.5（大预览浮层 + 键盘基础导航） |
| 最后更新 | 2026-09-23 |
| 当前阶段 | M0~M3 已完成（缩略图并发待优化）· M4 部分完成（滚动、键盘导航已做，搜索待做） |
...
| 2026-09-23 | v1.0.3 | **缩略图过期重抓 + 三个小开销项**：① 缩略图缓存带抓取时间，超过 TTL（10s）视为过期，收起后的补拍会重抓「缺失 + 过期」的窗口（ADR-043）——旧逻辑下已缓存的图永不更新，卡片停在旧帧甚至启动时的那一帧，与「实时窗口画面」的产品承诺不符；重抓仍只在面板不可见时进行（ADR-033），不在展开路径上加功耗。② 抓取进行中收到的补拍请求不再丢弃，任务结束后用最新窗口列表自动补一轮（旧逻辑下启动瞬间开新窗口会白卡到下次收起）。③ 部分抓取失败时 `lastError` 不再被无条件清空。④ 菜单栏图标改订阅 `PermissionsModel.objectWillChange`，删掉自己那个 2s 轮询 timer（轮询本就有，别两处各开一个）；⑤ 枚举时应用图标按 pid 在本次枚举内复用（`NSRunningApplication.icon` 每次调用都会重新渲染，多窗口应用从 N 次降为 1 次）；⑥ 删无调用且每刷必广播的 `WindowListModel.lastRefresh`，`diagnostics` 值不变不广播（`windows` 不能这样比：`WindowInfo` 的 `==` 只比 id）。新增 6 项新鲜度单测，单测 47 → 53 项 |
| 2026-09-23 | v1.0.4 | **修 R17「窗口多于 4 个时无任何溢出提示」**：采用方案①右缘露头——卡片行 leading 内边距改为由纯函数 `NotchGeometry.cardRowLeadingPadding` 反推（`展开宽 − visibleCount×步距 − peekWidth`，标准值 772−4×188−12 = 8，原为固定 16），offset=0 时第 5 张卡在右缘露出 12pt 窄条，提示「右边还有、可滚动」（ADR-044）。免费副产品：滚到尽头（maxOffset）右缘无露头，天然表示「到底了」。UI 常量不动（面板宽 772、卡片 176×132、步距 188、可视 4 张），纯布局重排，不影响滚动手感与既有几何单测。新增 3 项几何单测，单测 53 → 56 项 |
| 2026-09-23 | v1.0.5 | **落地 F4 大预览浮层（本项目核心差异点）+ F8 键盘基础导航**：① 悬停卡片 280ms（或键盘 ←→ 选中）后，预览带下方长出大预览卡——按窗口原始宽高比缩放的大图（最大 640×400）+ 信息行（图标/标题/应用名/最小化标记/✕ 关闭），材质与预览带同走 `kimiGlass`（ADR-045）；② 面板几何：预览可见时展开高度随之增长（`LargePreviewLayout.panelExtraHeight` 纯函数一处计算，控制器 frame 与 SwiftUI frame 同源消费），显示/隐藏分别用 .expand/.collapseHover 档位且两边同曲线（ADR-027 同理）；③ 缩略图抓取宽度 352→704px：卡片与大预览共用一份图，32 张约 40MB 仍在 64MB 预算内；④ 键盘：←→/↑↓ 移动选中（新 `PanelSelection.ensureVisible` 平滑滚动保证选中卡完整可见，已可见不吸附）、Return 打开、Esc 逐级收起（先隐预览再收面板）；只在展开且面板持 key 时接管（ADR-037 的 key 状态顺带成为键盘路由判据），⌘/⌥/⌃/⇧ 组合键原样放行；⑤ 滚动抑制悬停大预览（§6.5）；预览卡 ✕ 走 AX 关闭按钮；缩略图缺失时占位图保持同比例，面板高度不跳动。新增 19 项单测，56 → 75 项 |
...
6. **新鲜度（ADR-043，落地版与原计划不同）**：缓存图带抓取时间，超过 TTL（10s）视为过期；重抓时机不在展开时（ADR-033：面板在 level 1000 会挡住源窗口），而是在**收起后**与启动时补拍「缺失 + 过期」的窗口。不抓持续流、也不在展开路径上抓，功耗与实时性取中
...
│   ├── PanelFocus.swift                焦点状态：悬停/键盘选中、大预览延迟显隐（F4/F8）
│   ├── LargePreviewLayout.swift        大预览几何（纯函数：显示尺寸/卡尺寸/面板增高）
...
│       ├── LargePreviewCard.swift          大预览浮层（大图 + 信息行 + ✕ 关闭）
...
| 展开面板宽 | — | `16×2(内边距) + 4×176 + 3×12(间距)` = **772**（本机屏宽 1728，占比 45%）。v1.0.4（ADR-044）起卡片行内边距非对称：leading 8 + trailing 16，右缘留出第 5 张卡 12pt 露头，面板总宽不变 |
| 展开面板高 | — | 卡片 132 + 上下内边距 16 = **148**（面板整体 148 + 顶部留白 33 = 181） |
| 大预览 | — | 按窗口原始比例缩放，最大 640×400；下方信息行高 34（v1.0.5 落地，ADR-045：悬停 280ms 或键盘选中弹出，与预览带同面板、同材质，面板高度随之增长） |
| 溢出指示 | — | v1.0.4 以「右缘露头」回归（ADR-044，R17 关闭）：底部状态栏/计数仍不恢复，靠第 5 张卡的 12pt 露头提示可滚动 |
...
| ~~R17~~ | ~~可视区仅 4 张 → 窗口多时发现性下降~~ | ~~高~~ **已关闭（v1.0.4，ADR-044）** | 右缘露头已落地：收起位第 5 张卡露出 12pt 窄条提示可滚动 | 方案①（右缘露头）；② 遮罩与 ③ 首次提示未采用，理由见 ADR-044 |
...
| ADR-043 | **缩略图带 TTL 的过期重抓**：缓存图记录抓取时刻，超过 `staleInterval`（10s）视为过期；补拍范围从「缺失」扩为「缺失 + 过期」，时机不变（仅面板不可见时：启动 + 收起后 0.35s） | 旧逻辑只补「缺失」，已缓存的图**永不更新**：窗口内容变了卡片还停在旧帧，甚至停在启动那一刻——与 README「ScreenCaptureKit 抓的实时窗口画面」的承诺不符。备选方案对比：① 持续 `SCStream` 实时刷新——功耗不可接受（本项目底线，见 §4.4）；② 展开时重抓——ADR-033 已证明面板在 level 1000 会挡住源窗口，抓出来自带面板（「卡片上半发白」的原始 bug），不可行；③ AX 事件驱动失效（标题/frame 变化时单独失效）——覆盖不了画面本身的变化（纯内容改动不发 AX 通知），且链路复杂。TTL + 收起后补拍：时机现成（收起后本来就会调一次补拍）、抓取全程后台、面板快速开关因 TTL 自然去重（10s 内反复开关只抓一次），功耗增量可控（一轮全量重抓约几百 ms 的后台 GPU/IO，间隔以用户收起面板的节奏为限）。代价知情：TTL 内的画面变化要等下次收起补拍才可见，「实时」是近实时；`staleInterval = 0` 可退化为每次全量重抓。附带修复：抓取进行中收到的新补拍请求不再丢弃（记下待办，任务结束自动补一轮）；部分失败时 `lastError` 保留错误信息 | 2026-09-23 |
| ADR-044 | **溢出提示用「右缘露头」**：卡片行 leading 内边距由纯函数反推（展开宽 − 可视数×步距 − 露头宽），收起位保证第 5 张卡在右缘露出 12pt 窄条；不加任何覆盖层、不改面板/卡片尺寸 | R17 是面板精简（v0.8）遗留的高风险：>4 窗口时面板内无任何提示，用户不知道能滚。三个候选方案对比：① 右缘露头——滚动列表的原生惯例（iOS 分页点/各端 carousel），一眼读懂「还有内容」，且只动内边距，零新增视图与状态；② 两侧渐隐遮罩——要引入渐变覆盖层，且「减弱透明度」开啓时遮罩叠在不透明背景上语义混乱（ADR-034），另外它表达的是「此处被裁切」，不如露头表达「有东西等着你看」直接；③ 首次使用提示——需要持久化「看过没有」状态，且只教育一次，之后还是会忘。实现要点：露头宽度 12pt 是「圆角 10pt 的卡片能被认出是卡片」的最小经验值（再窄就只是一条线）；leading 内边距 16→8 是刻意的不对称，右侧的露头视觉上补齐了左侧收窄；offset=0 时露头有保证，中途自由滚动（ADR-023 无吸附）时裁切点连续扫过卡片，露头时有时无属预期；滚到 maxOffset 时右缘恰好无露头，成为「到底了」的天然信号——不额外做末端判定。代价知情：窄外接屏上展开宽不足 4×188+12 时内边距夹 0，露头消失（该场景本就连 4 张都放不下）；leading 8pt 与 §6.1 记录的「16×2 内边距」不再一致，规格表已同步 | 2026-09-23 |
| ADR-045 | **大预览浮层与预览带同处一个面板窗口**（预览可见时面板展开高度增长，宽度不变），不开第二个窗口；几何由纯函数 `LargePreviewLayout` 一处计算，控制器 frame 与 SwiftUI frame 同源消费；缩略图单份 704px 卡片与大预览共用；键盘导航只在「面板展开且持 key」时接管 ←→/Return/Esc | F4 是本项目的核心差异点，实现上最大的选型是「同一个窗口长高」还是「第二个浮窗」。选前者：层级（.screenSaver）、key 状态（液态玻璃的前提，ADR-037）、热区跟随、收起/挂起逻辑全部天然复用，不用维护第二套窗口生命周期与两窗间的 hover 衔接；代价是面板 frame 在预览显隐时要动画，但显示/隐藏本就有现成档位（.expand/.collapseHover），且 ADR-027 的「两边同档位同曲线」原则直接沿用。高度由窗口 frame 宽高比决定而不是缩略图是否已抓到——占位图保持同比例，面板高度不会因补拍完成而跳动。缩略图 352→704px：两份尺寸（卡片一份 + 预览一份）内存翻倍不可取，单份 704px 在卡片处下采样无损、在预览处（≤640pt）仅需轻度放大，32 张约 40MB 仍在 §4.10 的 64MB 预算内。键盘：ADR-037 让面板展开时持有 key，这恰好是「键盘归我们」的可靠判据——引导/调试窗口持 key 时面板不是 key，天然让路；只接管无修饰键的箭头/Return/Esc，⌘/⌥/⌃/⇧ 组合（含箭头自带的 .function 标志位）原样放行，不与系统热键/输入法冲突。ensureVisible 基于 ScrollFollow.target（渲染还没追上）而非 offset 判断，快速连按不会来回抖；已完整可见时不产生位移，保住 ADR-023「停位不被拉回」的语义。代价知情：预览图是缓存帧（ADR-033：面板可见时不抓，图里会带上自己的面板），「近实时」而非实时；`PanelFocus` 的悬停计时用 Task.sleep，退出路径靠 id 一致性校验兜底（SwiftUI onHover 离开/进入顺序不保证） | 2026-09-23 |
