...
- macOS 26+：优先用 Liquid Glass 材质（`.glassEffect` / `NSVisualEffectView` 的 `.hudWindow`/`.popover`），与 Tahoe 半透明菜单栏融合
- macOS 14/15：回退 `NSVisualEffectView(.hudWindow, .behindWindow)`
...
- 松手 120ms 后吸附到最近的 `cardStride` 边界，保证最终停位整齐
