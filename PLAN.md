...
| 项 | 值 |
|---|---|
| 文档版本 | v1.0.7（运行时内存占用优化） |
| 最后更新 | 2026-09-23 |
| 当前阶段 | M0~M3 已完成（缩略图并发待优化）· M4 部分完成（滚动已做，搜索待做；大预览按用户决定不做） |
...
| 2026-09-23 | v1.0.5 | ~~落地 F4 大预览浮层 + F8 键盘基础导航（ADR-045）~~ **v1.0.6 已整体回滚，见下行** |
| 2026-09-23 | v1.0.6 | **按用户实测反馈移除大预览浮层**：v1.0.5 落地后用户实测明确表示「不需要大预览图卡片」，当轮 revert 全部代码（ADR-045 随之作废，不保留在决策表中）。包括与之配套的两项：① 键盘 ←→/Return/Esc 导航——没有大预览后键盘选中无可见反馈，属半成品，一并撤掉，待 M4 搜索落地时重做；② 缩略图 352→704px——提分辨率是为大预览服务的，回 352 顺便省约 20MB 缓存。小卡片 + 滚动的既有形态保持不变；单测 75 → 56 项全绿。此条记录的价值：F4 是调研阶段认定的「核心差异点」，但真实用户（自己）用起来不需要——产品判断以实测体验为准，不以调研推演为准 |
| 2026-09-23 | v1.0.7 | **运行时内存占用优化（实测 footprint 26.0 → 25.1MB，IOSurface 2288K → 32K；多窗口/竖窗场景收益更大）**：① 缩略图只存「卡片实际显示的那块」——卡片是 16:10 中心裁切（`.fill`），入库前裁到同一区域并重画成紧凑位图（`bytesPerRow = 宽×4`），释放 ScreenCaptureKit 返回图的 IOSurface 背载；单张恒为 352×220×4 ≈ 300KB，上限 32 张 ≈ 9.6MB 可精确计算（旧实现存全幅，9:16 竖窗单张 ≈ 880KB）；② **收起动画结束后面板彻底离屏**（`orderOut`）：772×181@2x 透明窗口背后也是一整块可绘制内存，而面板绝大多数时间收起，离屏后 backing 由系统回收、合成器不再为它画透明像素；展开时 `orderFrontRegardless` 归位，悬停检测走全局 mouseMoved 不依赖窗口在屏，行为不变；LayerGuard 的重新归位也改成只在展开态执行，避免把离屏优化打穿；③ 订阅系统内存压力：warning 时缩略图缓存减半、critical 时全量释放 + `malloc_zone_pressure_relief`（纯缓存，下次收起后重抓即可）；④ 新增 `MemoryInfo`（phys_footprint，与活动监视器「内存」列同口径；RSS 含共享框架只读页会虚高到 80MB+，优化应盯 footprint），抓取批次完成后记入日志、调试面板新增「内存」节。新增 6 项裁切位图单测，56 → 62 项 |
...
| ~~F4~~ | ~~大预览~~ | ~~悬停卡片后延迟弹出大图，跟随键盘选择~~ **已按用户决定移除（v1.0.6，回滚 ADR-045）**：实测不需要；键盘导航随之撤下，待 M4 搜索时重做 |
...
│   ├── MemoryInfo.swift                进程物理占用读数（phys_footprint 口径）
│   ├── PrivateAPI/
...
| ~~大预览~~ | — | ~~按窗口原始比例缩放，最大 640×400；下方信息行高 34~~ 已按用户决定移除（v1.0.6） |
...
| ~~悬停卡片 → 弹出大预览~~ | ~~280ms~~ 已随大预览移除（v1.0.6） |
...
### M3 · 缩略图与大预览（3 天）— 09-27 ~ 10-02 · **缩略图完成；大预览已按用户决定不做（v1.0.6）**
...
- [x] ~~大预览浮层（延迟 280ms 弹出，跟随键盘选择）~~ **用户实测后决定不做**（v1.0.6 回滚，详见更新日志）；键盘导航一并撤下待 M4 重做
